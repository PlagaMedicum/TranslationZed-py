...
| 1. Git synchronization | Core preview in progress | Local committed baseline/ref/blob inspection, deterministic file/key/comment classification, immutable per-locale decisions, and format-aware explicit-Save insertion for JSON `NEW` rows exist with focused tests. Cache application, startup detection, and the synchronization UI remain. |
