...
C1 [→] **Translation memory** (Step 29).
   ...
   - **Scope now**:
    ...
    - TM import (TMX/XLIFF/XLF/PO/POT/CSV/MO) and TMX export for a **source+target locale pair** only.
    ...
    - [✓] Imported TM format support extended to `.xliff`/`.xlf`, `.po`/`.pot`, `.csv`, and `.mo` (alongside `.tmx`); drop-in sync scans all supported import extensions.
    ...
    - [ ] Add deferred import/export format adapters (XML, XLSX) behind the same import-workflow contract.
