...
C1 [→] **Translation memory** (Step 29).
   ...
   - **Scope now**:
    ...
    - TM import (TMX/XLIFF/XLF/PO/POT/CSV/MO/XML) and TMX export for a **source+target locale pair** only.
    ...
    - [✓] Imported TM format support extended to `.xliff`/`.xlf`, `.po`/`.pot`, `.csv`, `.mo`, and `.xml` (alongside `.tmx`); drop-in sync scans all supported import extensions.
