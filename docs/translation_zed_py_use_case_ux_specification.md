...
| **Notes** | Current active checks are `qa.trailing`, `qa.newlines`, and opt-in `qa.tokens` (`QA_CHECK_ESCAPES=true`); refresh is debounced on file-open/edit and also available by explicit QA-panel refresh action. If `QA_AUTO_MARK_FOR_REVIEW=true`, findings auto-mark affected rows to **For review**; default is visual-only. |
